...
static int  benchmark_MKL_SpMV ( const struct SparseMatrix *csrMatrix, double alpha, double *x,
                                 double beta, double *y, double *y_ref, double mflop, double matrixFrobeniusNorm, char* filename, int numIterations );
...
//    beta  = -M_PI / 3;
...
#ifdef _SpMV_MKL_
    __CHECK_ERROR_AND_CALL__ ( benchmark_MKL_SpMV ( &csrMatrix, alpha, x, beta, y, y_ref, mflop, matrixFrobeniusNorm , filename, numIterations),
    ...
    __CHECK_ERROR_AND_CALL__ ( benchmark_CSR_SpMV ( &csrMatrix, alpha, x, beta, y, y_ref, mflop, matrixFrobeniusNorm, 
                                                    INTEL_SPARSE_SCHEDULE_STATIC , filename, numIterations),
...
//                              0, "Benchmarking CSR SpMV with block scheduling failed\n" )
  ...
  {
    __CHECK_ERROR_AND_CALL__ ( benchmark_ESB_SpMV ( &csrMatrix, alpha, x, beta, y, y_ref, mflop, matrixFrobeniusNorm, 
                                                    INTEL_SPARSE_SCHEDULE_DYNAMIC , filename, numIterations ),
...
static void printPerformanceResults( const char* testName, double mflop, double bench_time, int niters, int schedule )
{
...
//    niters = 1000;
    ...
    printPerformanceResults( "MKL SPMV", mflop, time, niters, -1);
...
//    niters = 1000;
    ...
    time = time_end - time_start;
    // print performance results in GFlops and time per single SpMV call
    printPerformanceResults( "CSR SpMV", mflop, time, niters, schedule );
